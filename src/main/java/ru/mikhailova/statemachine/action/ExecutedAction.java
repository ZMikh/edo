...
import static ru.mikhailova.domain.statemachine.TaskState.EXECUTED;

@Slf4j
@RequiredArgsConstructor
public class ExecutedAction implements Action<TaskState, TaskEvent> {
    ...
    public void execute(StateContext<TaskState, TaskEvent> stateContext) {
        ...
        updateParam.setIsExecuted(true);
        updateParam.setTaskState(EXECUTED);
        taskService.updateTask(id, updateParam);
        log.info("From {} to {} task with id {}",
                stateContext.getSource().getId(), stateContext.getTarget().getId(), id);
    }
}
